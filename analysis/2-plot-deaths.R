...
df_cleaned <- df_input %>%
    mutate(
        ...
        time_to_coviddeath = if_else(
            is.na(date_death),
            as.Date("2020-10-01") - as.Date("2020-01-01"),
            as.Date(date_death) - as.Date("2020-01-01")
        ),
...
plot_deaths <- df_deathsperday %>%
    ggplot() +
    geom_area(
        aes(x = date_death, y = n, fill = age_group),
        stat = "identity",
        colour = "transparent"
    ) +
    ...
    labs(
        ...
        y = NULL,
        fill = NULL,
        title = "Daily deaths, covid versus non-covid"
    ) +
    scale_x_date(
        date_breaks = "1 month",
